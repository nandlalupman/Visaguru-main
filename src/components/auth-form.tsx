    ...
    <div className="space-y-4">
      ...
      {mode === "login" && (
        <div className="flex rounded-xl bg-[var(--color-surface)] p-1">
          ...
          <button
            type="button"
            onClick={() => { setAuthMethod("otp"); setError(null); setInfo(null); }}
            className={`flex-1 rounded-lg px-4 py-2 text-sm font-semibold transition ${
              authMethod === "otp"
                ? "bg-white text-[var(--color-navy)] shadow-sm"
                : "text-[var(--color-muted)] hover:text-[var(--color-navy)]"
      ...
      <AnimatePresence mode="wait">
        ...
        {/* ═══ OTP MODE ═══ */}
        {authMethod === "otp" && mode === "login" && otpStep === "email" && (
          <motion.form
            key="otp-email"
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        {authMethod === "otp" && mode === "login" && otpStep === "code" && (
          <motion.form
            ...
            transition={{ duration: 0.2 }}
            className="space-y-4"
            onSubmit={(e) => { e.preventDefault(); verifyOtpCode(); }}
          >
            ...
            <div className="flex justify-between text-sm">
              <button type="button" onClick={() => { setOtpStep("email"); setError(null); setInfo(null); }}
                className="text-[var(--color-muted)] hover:text-[var(--color-navy)] transition"
              >← Change email</button>
              <button type="button" disabled={isPending} onClick={sendOtp}
